...
class PageTextPreparation:
    ...
    def process_report(self, report_data):
        ...
        for page_content in self.report_data["content"]:
            ...
            if self._found_references and not page_text.strip():
                break
    ...
    def prepare_page_text(self, page_number):
        """主流程：处理页面块并组装为字符串。"""
        if getattr(self, "_found_references", False):
            return ""
    ...
    def _is_references_heading(self, text: str) -> bool:
        """Detect whether the given text marks the start of a references section."""
    ...
    def _apply_formatting_rules(self, blocks):
        ...
        while i < n:
            ...
            # Stop processing when hitting references section
            if block_type in {"section_header", "paragraph", "text"} and self._is_references_heading(text):
                self._found_references = True
                break
